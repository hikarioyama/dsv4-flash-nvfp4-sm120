...
class DeepseekV4FP8Config(Fp8Config):
    ...
    def get_quant_method(self, layer, prefix):
        if isinstance(layer, RoutedExperts):
            ...
            # MTP draft experts are stored as MXFP4 (group_size=32, e8m0
            # block scales) even when the main model uses NVFP4
            # (group_size=16, e4m3). The draft layer index is >=
            # num_hidden_layers; force MXFP4 there so the scale dtype and
            # group size match the checkpoint instead of being corrupted
            # by the NVFP4 loader's e8m0->e4m3 numeric conversion.
            is_draft_mtp = False
            try:
                hf_cfg = get_current_vllm_config().model_config.hf_config
                num_hidden = getattr(hf_cfg, "num_hidden_layers", 0)
                import re as _re
                ...
                pass
            if is_draft_mtp and self.expert_dtype == "fp4":
